...
public class CmdPat implements Command {
    ...
    private final CommandData slashInfo = new CommandData(this.getDescription().name(), this.getDescription().description())
            .addOptions(new OptionData(USER, optionName, "the person you want to pat")
                    .setRequired(false));

    public CmdPat(Hinata bot) {
        this.bot = bot;
    }
    ...
    public void runSlash(Guild guild, TextChannel tc, Member member, SlashCommandEvent event, InteractionHook hook) {
        ...
        if (event.getOption(this.optionName) != null) {
            ...
            if (member != target) {
                text = Objects.requireNonNull(target).getAsMention() + " you have been patted by **" + executor + "**!";
            } else {
                text = "*Pats* " + member.getAsMention();
            }
        } else {
            text = "*Pats* " + member.getAsMention();
        }
    ...
    public void runCommand(Message msg, Guild guild, TextChannel tc, Member member) {
        ...
        if (arguments.length > 0) {
            ...
            if (member != executor) {
                text = Objects.requireNonNull(member).getAsMention() + " you have been patted by **" + executorName + "**!";
            } else {
                text = "*Pats* " + member.getAsMention();
            }
        } else {
