...
import hinata.constants.ApiCalls;
...
@CommandDescription(
        name = "cuddle",
        description = "Cuddle someonee",
        triggers = {"cuddle"},
        attributes = {
                @CommandAttribute(key = "category", value = "reactions"),
                @CommandAttribute(key = "usage", value = "[command | alias]"),
                @CommandAttribute(key = "examples", value = "`h!cuddle 418037700751261708`\n" +
                        "`h!cuddle @Drag0n#6666`\n"),
        }
)
...
public class CmdCuddle implements Command {
    ...
    private final CommandData slashInfo = new CommandData(this.getDescription().name(), this.getDescription().description())
            .addOptions(new OptionData(USER, optionName, "the person you want to cuddle")
                    .setRequired(false));

    public CmdCuddle(Hinata bot) {
        this.bot = bot;
    }
    ...
    public void runSlash(Guild guild, TextChannel tc, Member member, SlashCommandEvent event, InteractionHook hook) {
        ...
        if (event.getOption(this.optionName) != null) {
            ...
            if (member != target) {
                text = Objects.requireNonNull(target).getAsMention() + " you have been cuddled by **" + executor + "**!";
            } else {
                text = "*Cuddles* " + member.getAsMention();
            }
        } else {
            text = "*Cuddles* " + member.getAsMention();
        }
    ...
    public void runCommand(Message msg, Guild guild, TextChannel tc, Member member) {
        ...
        if (arguments.length > 0) {
            ...
            if (member != executor) {
                text = Objects.requireNonNull(member).getAsMention() + " you have been cuddled by **" + executorName + "**!";
            } else {
                text = "*Cuddles* " + member.getAsMention();
            }
        } else {
