...
public class CmdPoke implements Command {
    ...
    private final CommandData slashInfo = new CommandData(this.getDescription().name(), this.getDescription().description())
            .addOptions(new OptionData(USER, optionName, "the person you want to poke")
                    .setRequired(false));

    public CmdPoke(Hinata bot) {
        this.bot = bot;
    }
    ...
    public void runSlash(Guild guild, TextChannel tc, Member member, SlashCommandEvent event, InteractionHook hook) {
        ...
        if (event.getOption(this.optionName) != null) {
            ...
            if (member != target) {
                text = Objects.requireNonNull(target).getAsMention() + " you have been pokes by **" + executor + "**!";
            } else {
                text = "*Pokes* " + member.getAsMention();
            }
        } else {
            text = "*Pokes* " + member.getAsMention();
        }
    ...
    public void runCommand(Message msg, Guild guild, TextChannel tc, Member member) {
        ...
        if (arguments.length > 0) {
            ...
            if (member != executor) {
                text = Objects.requireNonNull(member).getAsMention() + " you have been pokes by **" + executorName + "**!";
            } else {
                text = "*Pokes* " + member.getAsMention();
            }
        } else {
