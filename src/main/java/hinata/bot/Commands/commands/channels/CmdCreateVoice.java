...
@CommandDescription(
        ...
        triggers = {"createvoice", "cv", "newvoice"},
        attributes = {
                @CommandAttribute(key = "category", value = "channels"),
                @CommandAttribute(key = "usage", value = "[command | alias] [voice channel name]"),
                @CommandAttribute(key = "examples", value = "`h!cv info`\n"),
        }
)
public class CmdCreateVoice implements Command {
    ...
    private final CommandData slashInfo = new CommandData(this.getDescription().name(), this.getDescription().description())
            .addOptions(new OptionData(STRING, optionName, "the name of the new voice channel")
                    .setRequired(true));

    public CmdCreateVoice(Hinata bot) {
        this.bot = bot;
    ...
    public void runSlash(Guild guild, TextChannel tc, Member member, SlashCommandEvent event, InteractionHook hook) {
        ...
        embed.setColor(Colors.NORMAL.getCode())
                ...
                .setDescription("New voice channel created with name **" + name + "**!")
