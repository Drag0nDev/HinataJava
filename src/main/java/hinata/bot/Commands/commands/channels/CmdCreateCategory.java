...
@CommandDescription(
        ...
        triggers = {"createcategory", "cc", "newcategory"},
        attributes = {
                @CommandAttribute(key = "category", value = "channels"),
                @CommandAttribute(key = "usage", value = "[command | alias] [category name]"),
                @CommandAttribute(key = "examples", value = "`h!cc info`\n"),
        }
)
public class CmdCreateCategory implements Command {
    ...
    private final CommandData slashInfo = new CommandData(this.getDescription().name(), this.getDescription().description())
            .addOptions(new OptionData(STRING, optionName, "the name of the new category")
                    .setRequired(true));

    public CmdCreateCategory(Hinata bot) {
        this.bot = bot;
    ...
    public void runSlash(Guild guild, TextChannel tc, Member member, SlashCommandEvent event, InteractionHook hook) {
        ...
        embed.setColor(Colors.NORMAL.getCode())
                ...
                .setDescription("New category created with name **" + name + "**!")
