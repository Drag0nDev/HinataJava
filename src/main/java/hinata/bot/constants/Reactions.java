package hinata.bot.constants;

import java.util.Random;

public class Reactions {
    private final String[] HUG      = {"https://i.imgur.com/xELnNfC.gif",
                                    "https://i.imgur.com/nCwCS55.gif",
                                    "https://i.imgur.com/siVySzs.gif",
                                    "https://i.imgur.com/agdhkfE.gif",
                                    "https://i.imgur.com/Hpy7A01.gif",
                                    "https://i.imgur.com/VrETTlv.gif",
                                    "https://i.imgur.com/eKcWCgS.gif",
                                    "https://i.imgur.com/II1bakc.gif",
                                    "https://i.imgur.com/sGVgr74.gif",
                                    "https://i.imgur.com/TItLfqh.gif",
                                    "https://i.imgur.com/YbNv10F.gif",
                                    "https://i.imgur.com/e0ep0v3.gif",
                                    "https://i.imgur.com/8WKFiAa.gif",
                                    "https://i.imgur.com/wrDZ14I.gif"};

    private final String[] KISS     = {"https://i.imgur.com/04UFajc.gif",
                                    "https://i.imgur.com/gAcJs8p.gif",
                                    "https://i.imgur.com/NBqaFNq.gif",
                                    "https://i.imgur.com/7V3UQJ4.gif",
                                    "https://i.imgur.com/cwlKdT1.gif",
                                    "https://i.imgur.com/Jqd1H2l.gif",
                                    "https://i.imgur.com/28FHN4d.gif",
                                    "https://i.imgur.com/GMTkPNC.gif",
                                    "https://i.imgur.com/TfZriLL.gif",
                                    "https://i.imgur.com/A3aj2AF.gif",
                                    "https://i.pinimg.com/originals/38/93/c1/3893c18b81ef6e070f6a1c3a2f469d9d.gif"};

    private final String[] SLAP     = {"https://i.imgur.com/o5SrT0V.gif",
                                    "https://i.imgur.com/7CpOrix.gif",
                                    "https://i.imgur.com/g8A6W8t.gif",
                                    "https://i.imgur.com/53HB9hD.gif",
                                    "https://i.imgur.com/DSZkQIE.gif",
                                    "https://i.imgur.com/WqjeoMP.gif",
                                    "https://i.imgur.com/bI2zmE1.gif",
                                    "https://i.imgur.com/QRiFeeb.gif",
                                    "https://i.imgur.com/IgWcJ1k.gif",
                                    "https://i.imgur.com/F34jWzj.gif",
                                    "https://i.imgur.com/JAimthZ.gif",
                                    "https://i.imgur.com/0ngpRcS.gif",
                                    "https://i.imgur.com/fSSXGjj.gif",
                                    "https://i.imgur.com/O5GZyDU.gif",
                                    "https://i.imgur.com/ZxBRkvA.gif",
                                    "https://i.imgur.com/4Tqy1Jf.gif",
                                    "https://i.imgur.com/o6tU1i6.gif",
                                    "https://i.imgur.com/B85PHFs.gif",
                                    "https://i.imgur.com/9OIoIoP.gif",
                                    "https://i.imgur.com/pe9d4Fm.gif",
                                    "https://i.imgur.com/fuR2Pko.gif"};

    private final String[] BLUSH    = {"https://i.imgur.com/hJWIeMv.gif",
                                    "https://i.imgur.com/RHHL6aK.gif",
                                    "https://i.imgur.com/dXS8TDc.gif",
                                    "https://i.imgur.com/QkLsr6q.gif",
                                    "https://i.imgur.com/dH6yGFD.gif",
                                    "https://i.imgur.com/l5A8UJx.gif",
                                    "https://i.imgur.com/hlaBHi4.gif",
                                    "https://i.imgur.com/yxnYG9n.gif",
                                    "https://i.imgur.com/AQpwG9m.gif",
                                    "https://i.imgur.com/0b0optH.gif",
                                    "https://i.imgur.com/Vln95nw.gif",
                                    "https://i.imgur.com/mb0POnv.gif",
                                    "https://i.imgur.com/p6NkRgx.gif",
                                    "https://i.imgur.com/sUb0jK6.gif",
                                    "https://i.imgur.com/iN3g1iO.gif",
                                    "https://i.imgur.com/K9KHnbX.gif",
                                    "https://i.imgur.com/oi6bTZh.gif"};

    private final String[] GROPE    = {"https://i.imgur.com/lMNVEeS.gif",
                                    "https://i.imgur.com/2T4UY8Z.gif",
                                    "https://i.imgur.com/SICwu0O.gif",
                                    "https://i.imgur.com/z3zluLK.gif",
                                    "https://i.imgur.com/98jc66h.gif",
                                    "https://i.imgur.com/9m7msF5.gif",
                                    "https://i.imgur.com/nWeIr5x.gif",
                                    "https://i.imgur.com/1NJcHbo.gif",
                                    "https://i.imgur.com/Tf2rG7l.gif",
                                    "https://i.imgur.com/Tf2rG7l.gif",
                                    "https://i.imgur.com/vQ471NF.gif",
                                    "https://i.imgur.com/lGpvcIN.gif",
                                    "https://i.imgur.com/EbLmDx6.gif"};

    private final String[] CRY      = {"https://i.imgur.com/gD8wSJt.gif",
                                    "https://i.imgur.com/MSxWi8a.gif",
                                    "https://i.imgur.com/W4ulgbm.gif",
                                    "https://i.imgur.com/iEkGy0K.gif",
                                    "https://i.imgur.com/L7uGp75.gif",
                                    "https://i.imgur.com/itviV3b.gif",
                                    "https://i.imgur.com/lB8spOZ.gif",
                                    "https://i.imgur.com/GMy2eUD.gif",
                                    "https://i.imgur.com/pJuUyPC.gif",
                                    "https://i.imgur.com/sjrUvG6.gif",
                                    "https://i.imgur.com/KOUSm61.gif",
                                    "https://i.imgur.com/MOOn649.gif",
                                    "https://i.imgur.com/7tQDnEJ.gif",
                                    "https://i.imgur.com/4vwuGzK.gif",
                                    "https://i.imgur.com/zyKcpwu.gif",
                                    "https://i.imgur.com/lZOVeYQ.gif",
                                    "https://i.imgur.com/edbPydA.gif",
                                    "https://i.imgur.com/2JRSf41.gif"};

    private final String[] PAT      = {"https://i.imgur.com/o0RfGwO.gif",
                                    "https://i.imgur.com/MxesVdu.gif",
                                    "https://i.imgur.com/PHoLTsi.gif",
                                    "https://i.imgur.com/bTMdU8S.gif",
                                    "https://i.imgur.com/pGerQVB.gif",
                                    "https://i.imgur.com/tKVpqw9.gif",
                                    "https://i.imgur.com/NNOz81F.gif",
                                    "https://i.imgur.com/F3cjr3n.gif",
                                    "https://i.imgur.com/LUypjw3.gif",
                                    "https://i.imgur.com/XxJga2f.gif",
                                    "https://i.imgur.com/2k0MFIr.gif",
                                    "https://i.imgur.com/4ssddEQ.gif",
                                    "https://i.imgur.com/UWbKpx8.gif",
                                    "https://i.imgur.com/2lacG7l.gif",
                                    "https://i.imgur.com/1EgTn8z.gif"};

    public String getRandomHug() {
        Random rand = new Random();
        return this.HUG[rand.nextInt(this.HUG.length)];
    }

    public String getRandomkiss() {
        Random rand = new Random();
        return this.KISS[rand.nextInt(this.KISS.length)];
    }

    public String getRandomSlap() {
        Random rand = new Random();
        return this.SLAP[rand.nextInt(this.SLAP.length)];
    }

    public String getRandomBlush() {
        Random rand = new Random();
        return this.BLUSH[rand.nextInt(this.BLUSH.length)];
    }

    public String getRandomGrope() {
        Random rand = new Random();
        return this.GROPE[rand.nextInt(this.GROPE.length)];
    }

    public String getRandomCry() {
        Random rand = new Random();
        return this.CRY[rand.nextInt(this.CRY.length)];
    }

    public String getRandomPat() {
        Random rand = new Random();
        return this.PAT[rand.nextInt(this.PAT.length)];
    }
}